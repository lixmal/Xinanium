...
func New(name string, handle string, centric bool) *Player {
    ...
    if handle == "vik" {
        network.Write(commonnet.GET_PLAYER_TEX, nil)
        var playertex []byte

        action, buf, err := network.ReadAction()
        if err != nil || action != commonnet.PLAYER_TEX {
            log.Println("Unable to decode player sprite:", err)
            return nil
        }
        if err = network.DecodeBuf(&playertex, buf); err != nil {
            log.Println("Unable to decode player sprite:", err)
            return nil
        }
...
func (p *Player) Move(x, z float64) bool {
    ...
    if p.Speed > 0 && !p.Dead {
        ...
        if p.Handle == "vik" {
            network.Write(commonnet.PLAYER_MOVE, v)
        }
...
func (p *Player) SetPosition(v *lmath.Vec3, fromNetwork bool) bool {
